...
_.map = function(col, action){
    const result = [];
        _.each(col, function(element, index, arr){
            if (action(element, index, arr)){
...
_.every = function(collection, action) {
    let newArray = [];
  _.each(collection, function(element, index, container){
...
_.extend = function(obj1,...objs) {
   _.each(objs, function(element, index, objs) {
